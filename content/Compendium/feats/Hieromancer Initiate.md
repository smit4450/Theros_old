---
obsidianUIMode: preview
cssclasses:
- json5e-feat
tags:
- ttrpg-cli/compendium/src/5e/frhof
- ttrpg-cli/feat
aliases:
- Mythal Touched
---
# Hieromancer Initiate
*Source: FRHoF p. 40*  

**Prerequisite**: 4th

**Ability Score Increase.** Increase your Intelligence, Wisdom, or Charisma by 1, to a maximum of 20.

**Ward.** If a spell attack hits you or you fail a saving throw against a spell, you can take a [Reaction](Compendium/rules/variant-rules/reaction-xphb.md) to roll on the Hieromancer Initiate [Magic](Compendium/rules/actions.md#Magic) table to create a magical effect. If an effect requires a saving throw, the DC equals 8 plus the modifier of the ability increased by this feat and your [Proficiency Bonus](Compendium/rules/variant-rules/proficiency-xphb.md).

You can use this benefit a number of times equal to your [Proficiency Bonus](Compendium/rules/variant-rules/proficiency-xphb.md), and you regain all expended uses when you finish a [Long Rest](Compendium/rules/variant-rules/long-rest-xphb.md).

**Hieromancer Initiate Magic**

| dice: 1d20 | Effect |
|------------|--------|
| 1-2 | You and each creature within 15 feet of you make a Dexterity saving throw, taking Force damage equal to `1d8` times the level of the triggering spell on a failed save or half as much damage on a successful one. |
| 3-7 | You and the triggering spell's caster form a telepathic link for 1 hour. |
| 8-10 | Gravity is reversed in a 15-foot-radius, 60-foot-tall [Cylinder](Compendium/rules/variant-rules/cylinder-area-of-effect-xphb.md) centered on you for 1 minute, per the [Reverse Gravity](Compendium/spells/reverse-gravity-xphb.md) spell. |
| 11-13 | You and the triggering spell's caster each make a Constitution saving throw. On a failed save, the creature has the [Stunned](Compendium/rules/conditions.md#Stunned) condition until the end of its next turn. |
| 14-17 | You gain a +2 bonus to AC for 1 minute, potentially turning the triggering spell into a miss if it was a spell attack. |
| 18-19 | Any flammable, nonmagical object within 10 feet of the triggering spell's caster that isn't being worn or carried by another creature bursts into flame, takes `1d4` Fire damage, and is burning. |
| 20 | The triggering spell dissipates with no effect, and the action, [Bonus Action](Compendium/rules/variant-rules/bonus-action-xphb.md), or [Reaction](Compendium/rules/variant-rules/reaction-xphb.md) used to cast it is wasted. If that spell was cast with a spell slot, the slot isn't expended. |
^mythal-touched-magic